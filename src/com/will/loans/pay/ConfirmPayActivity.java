...
public class ConfirmPayActivity extends BasePayActivity implements
		OnClickListener {
	...
	protected String getMD5Code(Long time) {
		return GenerateMD5Password.encodeByMD5(SharePreferenceUtil.getUserPref(
				this).getToken()
				+ smf.format(time) + key);
	}
	...
	private void checkHaveTradePsw() {
		...
		try {
			...
			jo.put("userid",
					SharePreferenceUtil.getUserPref(ConfirmPayActivity.this)
							.getUserId());
			jo.put("token",
					SharePreferenceUtil.getUserPref(ConfirmPayActivity.this)
							.getToken());
		...
		aq.ajax(ServerInfo.HASTRADEPSW, params, JSONObject.class,
	...
	protected void afterSetContentView() {
		...
		nextBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (SharePreferenceUtil.getUserPref(ConfirmPayActivity.this)
						.getToken().equals("")) {
					startActivity(new Intent(ConfirmPayActivity.this,
							Register.class));
				} else {
					// checkHaveTradePsw();
					getData();
	...
	protected void getData() {
		...
		try {
			...
			jo.put("tradePsw", GenerateMD5Password.getMD5Password(mTradePsw
					.getText().toString()));
			String result = bankInfo.optString("cardtype");
			if (result.equals("1")) {
		...
		Map<String, String> params = new HashMap<String, String>();
		params.put("jsonData", jo.toString());
		aq.ajax(ServerInfo.BUYPRODUCTYB, params, JSONObject.class,
				new AjaxCallback<JSONObject>() {
					@Override
					public void callback(String url, JSONObject json,
							AjaxStatus status) {
...
//							finish();
	...
	protected void queryIsPaySuccess() {
		...
		Map<String, String> params = new HashMap<String, String>();
		params.put("jsonData", jo.toString());
		aq.ajax("http://daidaitong.imwanmei.com:8080/mobile/ybQueryOrder",
					...
					public void callback(String url, JSONObject json,
						...
						if (result.equals("0")) {
							...
							if (json.optString("status").equals("1")) {
								Toast.makeText(getApplication(), "支付成功",
										Toast.LENGTH_SHORT).show();
							} else {
								Toast.makeText(getApplication(), "支付失败",
										Toast.LENGTH_SHORT).show();
							}
							finish();
						} else if (result.equals("1")) {
	...
	private void updateView() {
		...
		setTextView(R.id.bank_card_num, "银行卡：" + product.optInt("startBuy")
				+ ".00元", "");
