...
public class FillVerifyCode extends BaseTextActivity {
	...
	public void buildParams() {
		...
		mAQuery.ajax(ServerInfo.REGISTERORLOGINBYMSG, params, JSONObject.class,
				mRegisterCallback);
	}
    ...
    AjaxCallback<JSONObject> mRegisterCallback = new AjaxCallback<JSONObject>() {
		...
		public void callback(String url, JSONObject object, AjaxStatus status) {
			...
			try {
				...
				if (state.equals("0")) {
					SharePreferenceUtil.getUserPref(FillVerifyCode.this)
							.setToken(object.optString("token"));
					SharePreferenceUtil.getUserPref(FillVerifyCode.this)
							.setUserId(object.optString("userid"));
					SharePreferenceUtil.getUserPref(FillVerifyCode.this)
							.setUsername(mNum);
					startActivity(new Intent(FillVerifyCode.this,
							SetPassword.class).putExtra(SetPassword.SETTYPE, 0));
					FillVerifyCode.this.finish();
				} else {
					Toast.makeText(getApplication(),
							object.optString("resultMsg"), 1 * 1000).show();
				}
			} catch (Exception e) {
	...
	private void init() {
		...
		((TextView) findViewById(R.id.title_tv))
				.setText(R.string.fill_verify_num);
